...
#define PC__VERSION 1
	...
	#define TLA(str) { char tlastmp[1024]; sprintf(tlastmp, "%s (file %s, line %d)", str, (unsigned char *)__FILE__, __LINE__);  throw(std::logic_error(tlastmp)); }
	#define TLA2(str1, str2) { char tlastmp[1024]; sprintf(tlastmp, "%s %s (file %s, line %d)", str1, str2, (unsigned char *)__FILE__, __LINE__);  throw(std::logic_error(tlastmp)); }
#ifndef _DEBUG
	#define TLL(condition, str) { if (condition) { char tlastmp[1024]; sprintf(tlastmp, " %s (file %s, line %d)", str, (unsigned char *)__FILE__, __LINE__);  throw(std::logic_error(tlastmp)); } }
#else
	#define TLL(condition, str) { if (condition) { char tlastmp[1024]; sprintf(tlastmp, " %s (file %s, line %d)", str, (unsigned char *)__FILE__, __LINE__);  mxat(false);  throw(std::logic_error(tlastmp)); } }
#endif
#endif
