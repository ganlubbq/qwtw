...
void IMatrixImpl::fromTextFile(char* fileName, XMType* x0, iiType h, iiType w) {
	...
	for ( i = 0; i < h; i++)  {
		for (j = 0; j < w; j++) {
			...
			if (k != 1) {
				//error
				xm_printf("\n IMatrixImpl::fromTextFile: file %s format error; i=%d; j = %d", 
					fileName, i, j);
				break;
			}
		}
...
#ifdef _DEBUG
void IMatrixImpl::print(const XMType* x, iiType hh, iiType ww, unsigned short  p) {
	unsigned short  i,j;
	char	s1[80];
	char	s2[16];
	...
	if (name[0] == 0) {
		...
		memcpy(name, "noname\0  ", 7); 
	}

	sprintf(s1, "   matrix  %s %d x %d :  \n", name, hh, ww);
	xm_printf(s1);
	...
	for(i=0;i<hh;i++) {
		for(j=0;j<ww;j++) {
			sprintf(s1, s2, x[i*ww + j]);
			xm_printf(s1);
			//xm_printf(s1, s2, x0[i*w0 + j]);
		}
		if (i == (hh - 1)) { xm_printf("];");} else { xm_printf(";"); }
		xm_printf("\n"); 
	}
	xm_printf("\n"); 
}
